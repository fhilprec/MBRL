...
class AtariWrapper(GymnaxWrapper):
    ...
    @functools.partial(jax.jit, static_argnums=(0,))
    def step(
        self, key: chex.PRNGKey, state: AtariState, action: Union[int, float]
    ) -> Tuple[chex.Array, EnvState, float, bool, Dict[Any, Any]]:
        new_action = action
        if self.sticky_actions:
            # With probability 0.25, we repeat the previous action
            key, repeat_key = jax.random.split(key)
            repeat_prev_action_mask = (
                jax.random.uniform(repeat_key, shape=action.shape) < 0.25
            )
        ...
        info = {k: reduce_info(k, v) for k, v in infos._asdict().items()}
...
class MultiRewardLogWrapper(GymnaxWrapper):
    ...
    def reset(
        ...
        state = MultiRewardLogEnvState(
            env_state, 0, episode_returns_init, 0, 0, episode_returns_init, 0
        )
        return obs, state
