...
def PongLSTM():
    def forward(state, action, lstm_state=None):
        ...
        action_one_hot = jax.nn.one_hot(action, num_classes=6)  # Pong has 6 discrete actions
...
def PongDreamer():
    ...
    def forward(state, action, dreamer_state=None):
        ...
        flat_state_normalized = jnp.tanh(flat_state_clipped / 50.0)  # Normalize to [-1, 1]
...
def PongLSTMFixed():
    ...
    def forward(state, action, lstm_state=None):
        ...
        flat_state = jnp.clip(flat_state, jnp.percentile(flat_state, 1), 
                             jnp.percentile(flat_state, 99))
