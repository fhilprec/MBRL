...
def compare_real_vs_model(
    ...
    def debug_obs(
        ...
        print(
            f"Step {step}, Unnormalized Error: {error:.2f} | Action: {action_map.get(int(action), action)} Reward : {get_simple_dense_reward(real_obs, action, frame_stack_size=frame_stack_size):.2f}"
        )
    ...
    while step_count < min(num_steps, len(obs) - 1):
        ...
        print(f"Reward : {get_simple_dense_reward(obs[step_count + 1], action, frame_stack_size=frame_stack_size):.2f}")
