...
(TODO SOURCE). Also, the lack of a model can make it difficult for the
...
agent more robust (TODO SOURCE). Moreover they provide limited
...
environment (TODO SOURCE). Moreover there are approaches that use
...
(TODO SOURCE) is a model-based RL algorithm. In general, Dreamer
...
input to create objects from it (TODO SOURCE) but
also approaches where the objects states are extracted from RAM
(TODO SOURCE) or given directly by the
...
\begin{itemize}
	...
	\item \textbf{Sample Inefficiency:} Learning from high-dimensional pixel data often requires more samples to achieve comparable performance to object-centric approaches (TODO SOURCE).
...
understands the concept of a "ball" an an abstract object can use this knowledge in environments where the ball has a different shape or appearance in general (TODO SOURCE).
...
pixel data, agents can learn policies with fewer rollouts. This is possible because the object space is already some kind of latent space which is usually the way things work (TODO SOURCE).

A significant development in this field is the introduction of OCAtari
(Object-Centric Atari) by Delfosse et al.
(TODO SOURCE). OCAtari extends the
...
Building upon OCAtari, JAXAtari (TODO SOURCE) provides a JAX-based
...
\textbf{Causality:} (TODO SOURCE) Recognizing cause-and-effect relationships between actions and object state changes. This understanding enables more sophisticated planning and can help avoid unintended consequences.
